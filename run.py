...
def change_img_index(x):
    ...
    print('name of selected image: ', img_path)
    if WITH_QT:
...
def draw_text(tmp_img, text, center, color, size):
    ...
    cv2.putText(tmp_img, text, center, font, 0.5, color, 1, cv2.FONT_HERSHEY_COMPLEX_SMALL)
    return tmp_img
...
def set_selected_bbox():
    ...
    for idx, obj in enumerate(img_objects):
        ...
        if is_mouse_inside_points(x1, y1, x2, y2):
            ...
            #cv2.putText(tmp_img, "chosen box", (x1+x2)/2, (y1 +y2)/2, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.FONT_HERSHEY_COMPLEX_SMALL)
...
#SORT OR NOT?
def sort_criteria(strs):
  notd=-1
  print(strs[7:])
  for i, c in enumerate(strs[7:]):
    if not c.isdigit():
        notd=i
        break
  return eval(strs[7:7+notd])

image_list.sort(key=sort_criteria)
#if not args.sort:
...
class_rgb = [
    ...
    (255, 0, 255), (192, 192, 192), (45,82,160), (128, 0, 0),
    (128, 128, 0), (0, 128, 0), (128, 0, 128), (0, 128, 128), (0, 0, 128),(0, 191, 255),(0,140,255),(250,230,230),(221,160,221)]
...
''' declare all callback of button
+++++++++++++++++++++++++++++++++++++++++
+++++++++++++++++++++++++++++++++++++++++
+++++++++++++++++++++++++++++++++++++++++
'''
def update_to_number(number):
    if len(class_list)>=number+1:
	    class_index=number
	    color = class_rgb[class_index].tolist()
...
##########declare button
'''
cv2.createButton("0",b0,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("1",b1,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("2",b2,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("3",b3,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("4",b4,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("5",b5,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("6",b6,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("7",b7,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("8",b8,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("9",b9,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("10",b10,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("11",b11,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("12",b12,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("13",b13,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("14",b14,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("15",b15,None,cv2.QT_PUSH_BUTTON,0)
cv2.createButton("16",b16,None,cv2.QT_PUSH_BUTTON,0)
'''
...
while True:
    ...
    plus=20
    if (mouse_x+plus<width):
       cv2.line(tmp_img, (mouse_x,mouse_y), (mouse_x+plus,mouse_y), (0, 255, 0), 1)
    if (mouse_y+plus<height):
       cv2.line(tmp_img, (mouse_x,mouse_y), (mouse_x,mouse_y+plus), (0, 255, 0), 1)
    if (mouse_x-plus>0):
       cv2.line(tmp_img, (mouse_x,mouse_y), (mouse_x-plus,mouse_y), (0, 255,0), 1)
    if (mouse_y-plus>0):
       cv2.line(tmp_img, (mouse_x,mouse_y), (mouse_x,mouse_y-plus), (0, 255, 0), 1)
